...
const DashboardOverview = () => {
    ...
    return (<div>
        <header className="flex items-center justify-between mt-0 bg-[#192218] py-2 px-4">
            <div>
                <h2 className="text-lg md:text-2xl font-bold text-white"><span className="text-[#14A800] font-medium">Welcome</span> {user.displayName}</h2>
            </div>
          ...
          >
            {theme === "light" ? "🌙" : "☀️"}
                ...
                <img src={user.photoURL} alt="" className="w-10 h-10 rounded-full object-cover cursor-pointer"/>
                <button onClick={logOutUser} className="text-[#14A800] font-medium ml-2 border-2 border-[#14A800] px-4 py-2 rounded-lg cursor-pointer">Logout</button>
            </div>
        </header>
        <div>
